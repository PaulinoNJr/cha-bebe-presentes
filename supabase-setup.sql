...
create table if not exists public.gift_reservations (
  ...
  reserved_by_cpf text not null check (reserved_by_cpf ~ '^[0-9]{11}$'),
...
begin
  ...
  insert into public.gift_reservations (gift_id, reserved_by, reserved_by_cpf, qty)
  values (p_gift_id, v_name, v_cpf, v_qty);
...
begin
  ...
  if coalesce(v_saved_cpf, '') <> v_cpf then
...
revoke select(reserved_by_cpf) on public.gift_reservations from anon, authenticated;
